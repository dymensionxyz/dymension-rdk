...
func GetTransferSenderRecipient(packet channeltypes.Packet) (
	...
	if err := transfertypes.ModuleCdc.UnmarshalJSON(packet.GetData(), &data); err != nil {
		return nil, nil, "", "", errortypes.Wrapf(errortypes.ErrUnknownRequest, "cannot unmarshal ICS-20 transfer packet data")
	}
	...
	if err != nil {
		return nil, nil, "", "", errortypes.Wrap(err, "invalid sender")
	}
	...
	if err != nil {
		return nil, nil, "", "", errortypes.Wrap(err, "invalid recipient")
	}
...
func GetTransferAmount(packet channeltypes.Packet) (string, error) {
	...
	var data transfertypes.FungibleTokenPacketData
	if err := transfertypes.ModuleCdc.UnmarshalJSON(packet.GetData(), &data); err != nil {
		return "", errortypes.Wrapf(errortypes.ErrUnknownRequest, "cannot unmarshal ICS-20 transfer packet data")
	}

	if data.Amount == "" {
		return "", errortypes.Wrapf(errortypes.ErrInvalidCoins, "empty amount")
	}

	if _, ok := sdk.NewIntFromString(data.Amount); !ok {
		return "", errortypes.Wrapf(errortypes.ErrInvalidCoins, "invalid amount")
	}
