...
import (
	...
	ibctypes "github.com/dymensionxyz/rollapp/x/erc20middleware/types"
)
...
func (k Keeper) OnRecvPacket(
	...
	if err := transfertypes.ModuleCdc.UnmarshalJSON(packet.GetData(), &data); err != nil {
		...
		err = errortypes.Wrapf(errortypes.ErrInvalidType, "cannot unmarshal ICS-20 transfer packet data")
	...
	sender, recipient, _, _, err := ibctypes.GetTransferSenderRecipient(packet)
	if err != nil {
		return channeltypes.NewErrorAcknowledgement(err.Error())
	}
	...
	if ibctypes.IsModuleAccount(senderAcc) {
		return ack
	}

	// parse the transferred denom
	coin := ibctypes.GetReceivedCoin(
	...
	pairID := k.erc20Keeper.GetTokenPairID(ctx, coin.Denom)
	if len(pairID) == 0 {
	...
	if _, err = k.erc20Keeper.ConvertCoin(sdk.WrapSDKContext(ctx), msg); err != nil {
		return channeltypes.NewErrorAcknowledgement(err.Error())
	}
...
func (k Keeper) ConvertCoinToERC20FromPacket(ctx sdk.Context, data transfertypes.FungibleTokenPacketData) error {
	...
	coin := ibctypes.GetSentCoin(data.Denom, data.Amount)
