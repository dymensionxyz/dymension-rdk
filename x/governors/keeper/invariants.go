...
func ModuleAccountInvariants(k Keeper) sdk.Invariant {
	return func(ctx sdk.Context) (string, bool) {
		...
		k.IterateGovernors(ctx, func(_ int64, governor types.GovernorI) bool {
			switch governor.GetStatus() {
			case types.Bonded:
				bonded = bonded.Add(governor.GetTokens())
			case types.Unbonding, types.Unbonded:
				notBonded = notBonded.Add(governor.GetTokens())
			default:
		...
		// Bonded tokens should equal sum of tokens with bonded governors
		// Not-bonded tokens should equal unbonding delegations	plus tokens on unbonded governors
...
// amount stored in each governor.
func DelegatorSharesInvariant(k Keeper) sdk.Invariant {
	return func(ctx sdk.Context) (string, bool) {
		...
		governorsDelegationShares := map[string]sdk.Dec{}

		// initialize a map: governor -> its delegation shares
		for _, governor := range governors {
			governorsDelegationShares[governor.GetOperator().String()] = sdk.ZeroDec()
		}
		...
		// for each governor, check if its total delegation shares calculated from the step above equals to its expected delegation shares
		for _, governor := range governors {
			expValTotalDelShares := governor.GetDelegatorShares()
			calculatedValTotalDelShares := governorsDelegationShares[governor.GetOperator().String()]
			if !calculatedValTotalDelShares.Equal(expValTotalDelShares) {
				broken = true
				msg += fmt.Sprintf("broken delegator shares invariance:\n"+
					"\tgovernor.DelegatorShares: %v\n"+
